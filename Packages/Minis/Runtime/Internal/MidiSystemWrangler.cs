...
static class MidiSystemWrangler
{
    ...
    static MidiSystemWrangler()
    {
        ...
        // Aetherin only consumes MIDI while playing. Updating the native RtMidi
        // driver from EditorApplication.update caused it to run in Edit Mode and
        // twice per frame during Play Mode, which can stall the whole Editor when
        // the Windows MIDI backend blocks. The PlayerLoop callback above is the
        // sole update path while playing.
